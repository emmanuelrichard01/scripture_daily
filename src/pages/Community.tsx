...
export default function Community() {
  ...
  // Realtime subscription for instant social updates
  useEffect(() => {
    if (!userId) return;
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const newNudge = payload.new as {
            sender_name?: string;
            message?: string;
            scripture_reference?: string;
          };
          const title = newNudge.sender_name
            ? `${newNudge.sender_name} cheered you on!`
            : "You received an encouragement!";
          const description =
            newNudge.scripture_reference || newNudge.message || "Keep up your reading streak today.";
  ...
  const relationshipState = useMemo(() => {
    ...
    for (const friend of friends.data ?? []) {
      map.set(friend.profile.userId, "friend");
    }
    for (const req of outgoing.data ?? []) {
      map.set(req.profile.userId, "requested");
    }
    for (const req of incoming.data ?? []) {
      map.set(req.profile.userId, "incoming");
    }
    return map;
  ...
  return (
    <PageLayout title="Community">
      ...
      {invited.data && (
        <section className="surface mb-5 p-4 sm:p-5">
          <div className="flex items-center gap-3.5">
            <Avatar className="h-11 w-11 shrink-0 border border-card shadow-xs">
          ...
          <div className="mt-3.5 flex gap-2">
            {!invitedState && (
              <Button
                ...
                className="h-10 flex-1 gap-2 rounded-xl text-xs font-bold bg-primary text-primary-foreground hover:opacity-90 cursor-pointer"
              >
            ...
            <Button
              ...
              className={cn("h-10 rounded-xl text-xs cursor-pointer", !invitedState ? "px-3" : "flex-1")}
            >
              Dismiss
            </Button>
          </div>
        ...
        <div className="space-y-5">
          ...
          >
              ...
              <div>
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <h2 className="font-display text-base sm:text-lg font-bold leading-snug">
                    ...
                    <div className="flex shrink-0 -space-x-2" aria-hidden="true">
                      {activeToday.slice(0, 4).map((friend) => (
                        <Avatar
                          ...
                          className="h-9 w-9 border-2 border-card shadow-xs"
                        >
                      ...
                      {activeToday.length > 4 && (
                        <span className="flex h-9 w-9 items-center justify-center rounded-full border-2 border-card bg-secondary text-2xs font-bold shadow-xs">
                          +{activeToday.length - 4}
                        </span>
                      )}
                    </div>
                ...
                <div className="mt-4 grid grid-cols-3 divide-x divide-border/60 rounded-xl bg-secondary/40 py-3">
                  {[
                    ...
                    <div key={stat.label} className="px-2 text-center">
                      <p className="font-display text-lg sm:text-xl font-bold leading-none">{stat.value}</p>
                      <p className="mt-1 text-[0.62rem] font-semibold uppercase tracking-[0.06em] text-muted-foreground">
                        {stat.label}
                      </p>
                    </div>
                  ))}
            ...
            <div className="mt-4 flex gap-2">
              <Button
                ...
                className={cn(
                  "h-11 flex-1 gap-2 rounded-xl text-xs font-bold cursor-pointer",
                  !hasCircle && "bg-primary text-primary-foreground hover:opacity-90",
                )}
              >
          ...
          {hasCircle && (
            <section aria-labelledby="standings-heading">
              <h2 id="standings-heading" className="section-label mb-2.5 flex items-center gap-1.5">
                <Trophy className="h-3.5 w-3.5 text-track-yellow" aria-hidden="true" />
                <span>Circle Standings</span>
              </h2>
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        index === 0
                          ? "text-track-yellow font-extrabold"
                          : index === 1
                    ...
                    <span className="relative shrink-0">
                      {row.profile ? (
                        ...
                        <span className="flex h-8.5 w-8.5 items-center justify-center rounded-full bg-primary/15 text-2xs font-bold text-primary">
                          You
                        </span>
                      )}
                      {row.readToday && (
                    ...
                    <span
                      className={cn(
                        ...
                        row.isSelf ? "font-bold text-foreground" : "font-medium text-foreground",
                      )}
                    >
                      {row.name}
                    </span>
                    ...
                    <span className="w-16 shrink-0 text-right text-xs sm:text-sm font-bold tabular-nums">
                      {row.chapters.toLocaleString()} <span className="text-[0.62rem] font-normal text-muted-foreground">ch</span>
                    </span>
                  </li>
                ))}
              </ol>
      ...
      {activeTab === "friends" && (
        <section aria-labelledby="friends-heading" className="space-y-3.5">
          <div className="flex items-center justify-between">
            <h2 id="friends-heading" className="section-label flex items-center gap-1.5">
              <Users className="h-3.5 w-3.5" aria-hidden="true" />
              <span>{friendList.length} {pluralize(friendList.length, "Friend")}</span>
            </h2>
            <Button
              size="sm"
              variant="ghost"
              onClick={() => setActiveTab("discover")}
              className="h-8 gap-1 rounded-xl text-xs font-semibold text-primary hover:text-primary cursor-pointer"
            >
          ...
          {friends.isPending ? (
            ...
            <ul className="space-y-2">
              {friendList.map((friend) => (
                <li
                  key={friend.friendshipId}
                  className="surface p-3.5 transition-colors hover:bg-secondary/20"
                >
                  <div className="flex items-center justify-between gap-3">
                    <button
                      type="button"
                      onClick={() => setSelectedFriend(friend)}
                      className="flex min-w-0 flex-1 items-center gap-3 text-left focus-ring rounded-xl cursor-pointer"
                    >
                      <div className="relative shrink-0">
                        <Avatar className="h-10 w-10 border border-card shadow-xs">
                          <AvatarImage src={friend.profile.avatarUrl ?? undefined} alt="" />
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      <Button
                        ...
                        className="h-8 gap-1.5 rounded-lg px-2.5 text-xs font-semibold text-primary hover:bg-primary/10 cursor-pointer"
                      >
                      ...
                      <Button
                        ...
                        className="h-8 w-8 rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive cursor-pointer"
                  ...
                  <div className="mt-2.5 flex items-center gap-3 border-t border-border/50 pt-2 text-[0.68rem] text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <BookOpen className="h-3 w-3" />
                      <span className="font-bold tabular-nums text-foreground">{friend.chapters.toLocaleString()}</span> chapters
                    </span>
            ...
            <div className="rounded-2xl border border-dashed border-border p-8 text-center">
              <Sparkles
                className="mx-auto mb-2.5 h-8 w-8 text-muted-foreground/40"
                strokeWidth={1.5}
                aria-hidden="true"
              />
              <p className="text-xs sm:text-sm font-bold">No friends connected yet</p>
              <p className="mx-auto mt-1 max-w-xs text-xs leading-relaxed text-muted-foreground">
                Invite friends via link or scan their QR code to cheer each other on through the Scriptures.
              </p>
              <Button
                onClick={() => setActiveTab("discover")}
                className="mt-3.5 h-10 rounded-xl px-4 text-xs font-bold bg-primary text-primary-foreground hover:opacity-90 cursor-pointer"
              >
                Find & Add Friends
              </Button>
            </div>
      ...
      {activeTab === "requests" && (
        <div className="space-y-5">
          ...
          <section aria-labelledby="incoming-requests">
            <h2 id="incoming-requests" className="section-label mb-2.5 flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-track-orange" aria-hidden="true" />
              <span>Incoming Requests ({incoming.data?.length ?? 0})</span>
            </h2>
                  ...
                  >
                    ...
                    <div className="flex shrink-0 gap-1.5">
                      <Button
                        ...
                        className="h-8 gap-1 rounded-lg bg-primary px-3 text-xs font-bold text-primary-foreground hover:opacity-90 cursor-pointer"
                      >
                      ...
                      <Button
                        ...
                        disabled={respond.isPending}
                        className="h-8 rounded-lg px-2 text-xs text-muted-foreground hover:text-destructive cursor-pointer"
                      >
                        <span>Decline</span>
                      </Button>
                    </div>
          ...
          <section aria-labelledby="outgoing-requests">
            <h2 id="outgoing-requests" className="section-label mb-2.5">
                ...
                {outgoing.data.map((request) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      className="h-8 shrink-0 rounded-lg text-xs text-muted-foreground hover:text-destructive cursor-pointer"
                    >
                      Withdraw
                    </Button>
                  </li>
        ...
        <div className="space-y-5">
          ...
          <div className="surface flex items-center justify-between p-4 sm:p-5">
            <div className="min-w-0 pr-2">
              <h3 className="font-display text-sm sm:text-base font-bold">Connect via QR Code</h3>
              <p className="mt-0.5 text-xs text-muted-foreground">
            ...
            <Button
              onClick={() => setIsQRModalOpen(true)}
              className="h-10 gap-1.5 rounded-xl px-3.5 text-xs font-bold bg-primary text-primary-foreground hover:opacity-90 cursor-pointer"
            >
          ...
          {/* Search Box */}
          <div>
            <h2 className="section-label mb-2">Search Readers</h2>
            <div className="relative mb-3.5">
              ...
              <Input
                ...
                className="h-11 rounded-xl border-border/70 bg-card pl-10 text-xs sm:text-sm"
              />
            </div>
              ...
              <section aria-label="Search results">
                {search.isPending ? (
                        ...
                        >
                          ...
                          {state === "friend" ? (
                            ...
                            <Button
                              ...
                              disabled={add.isPending}
                              className="h-8 shrink-0 gap-1 rounded-lg text-xs font-bold bg-primary text-primary-foreground hover:opacity-90 cursor-pointer"
                            >
                  ...
                  <div className="rounded-2xl border border-dashed border-border p-6 text-center">
                    <p className="text-xs sm:text-sm font-semibold">No readers found</p>
                    <p className="mx-auto mt-1 max-w-xs text-xs leading-relaxed text-muted-foreground">
      ...
      >
        <AlertDialogContent className="max-w-sm rounded-2xl">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="h-10 rounded-xl text-xs cursor-pointer">Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => pendingRemoval && remove.mutate(pendingRemoval.friendshipId)}
              className="h-10 rounded-xl text-xs bg-destructive text-destructive-foreground hover:bg-destructive/90 cursor-pointer"
            >
              Remove
            </AlertDialogAction>
          </AlertDialogFooter>
...
function PersonRow({ profile, muted = false }: { profile: CommunityProfile; muted?: boolean }) {
  return (
    <div className="flex min-w-0 items-center gap-2.5">
      <Avatar className="h-9 w-9 shrink-0 border border-card shadow-xs">
        <AvatarImage src={profile.avatarUrl ?? undefined} alt="" />
        <AvatarFallback className="text-2xs font-bold">{initialsOf(profile)}</AvatarFallback>
      </Avatar>
      <span
        className={cn(
          "min-w-0 truncate text-xs sm:text-sm font-semibold",
